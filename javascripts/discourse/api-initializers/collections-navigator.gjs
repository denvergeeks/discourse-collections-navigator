  ...
  api.onPageChange((url) => {
    ...
    waitForCollectionsDom((sidebarPanel, postsContainer) => {
      ...
      currentCleanup = () => {
        ...
        modalRequestId++;
       Absolutely — here is the **full updated contents** of `javascripts/discourse/api-initializers/collections-navigator.gjs` with the mobile-initialization fix merged in, so the collection navigator controls initialize correctly below 768px without requiring a resize first. Using `api.onPageChange` inside an `apiInitializer` is the current Discourse-supported pattern for this sort of theme/component behavior.[web:3][web:42]
          ...
          <div class="modal-body-split">
            <div class="modal-items-sidebar collapsed">
              <ul class="collection-items-list">
                ${items
                  .map((item, idx) =>
                    renderCollectionItem(item, idx, currentIndex)
                  )
                  .join("")}
              </ul>
            </div>
